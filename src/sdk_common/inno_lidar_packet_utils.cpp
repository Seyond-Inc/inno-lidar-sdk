...
inline void InnoDataPacketUtils::lookup_xyz_adjustment_(const InnoBlockAngles &angles, uint32_t scan_id,
                                                        uint32_t radius_unit, double adj[],
                                                        InnoItemType type) {
...
void InnoDataPacketUtils::get_xyzr_meter(const InnoBlockAngles angles, const uint32_t radius_unit,
  ...
  if (type == INNO_ITEM_TYPE_SPHERE_POINTCLOUD || type == INNO_FALCONII_DOT_1_ITEM_TYPE_SPHERE_POINTCLOUD) {
    ...
    // Todo add nps adjustment for robine2
    return;  // no adjustment for HB compact pointcloud
  } else if (type == INNO_ROBINW_ITEM_TYPE_SPHERE_POINTCLOUD || CHECK_CO_SPHERE_POINTCLOUD_DATA(type)) {
    double adj[3];
    lookup_xyz_adjustment_(angles, channel, radius_unit, adj, type);
    result->x += adj[0];
    result->y += adj[1];
    result->z += adj[2];
  }
...
bool InnoDataPacketUtils::convert_to_xyz_pointcloud(const InnoDataPacket &src, InnoDataPacket *dest, size_t dest_size,
                                                    ...
                                                    const char *hvangle_table, bool append) {
  ...
  if (src.type == INNO_ITEM_TYPE_SPHERE_POINTCLOUD) {
    ...
    dest->item_size = sizeof(InnoEnXyzPoint);
  } else if (src.type == INNO_ROBINE2_ITEM_TYPE_COMPACT_POINTCLOUD) {
    dest->type = INNO_ROBINE2_ITEM_TYPE_XYZ_POINTCLOUD;
    dest->item_size = sizeof(InnoEnXyzPoint);
  } else if (src.type == INNO_ROBINE2X_ITEM_TYPE_COMPACT_POINTCLOUD) {
    dest->type = INNO_ROBINE2X_ITEM_TYPE_XYZ_POINTCLOUD;
    dest->item_size = sizeof(InnoEnXyzPoint);
  } else {
...
#define CONVERT_EN_FN(ctx, p, blk, pt, full_angles, ch, m)                                          \
  do {                                                                                              \
    if (pt.radius > 0) {                                                                            \
      ...
      ipt.multi_return = m;                                                                         \
      ipt.is_2nd_return = pt.is_2nd_return;                                                         \
      ipt.timestamp_s = ipt.ts_10us / k10UsInSecond + src.common.ts_start_us / kUsInSecond;       \
      ipt.ts_10us += time_adjust_10us;                                                              \
      dest->item_number++;                                                                          \
    }                                                                                               \
  } while (0)
...
int InnoDataPacketUtils::printf_status_packet(const InnoStatusPacket &pkt, char *buffer, size_t buffer_size) {
  ...
  int ret_c =
      snprintf(buffer, buffer_size,
               ...
               " in_faults_extended_faults=0x%u"";",
               ...
               // in_faults
               pkt.in_faults.faults, pkt.in_faults.extended_faults);
...
bool InnoDataPacketUtils::check_status_packet_fault(const InnoStatusPacket &pkt) {
  if (pkt.in_faults.faults || pkt.in_faults.extended_faults || pkt.ex_faults.faults) {
    return true;
  }
  return false;
}
...
bool InnoDataPacketUtils::check_status_packet_fault_id(const InnoStatusPacket &pkt, int fid) {
  ...
  } else if (fid >= 96 && fid < 127) {  // fid 96 - 127 store in pkt.ex_faults
      ret = pkt.ex_faults.faults & (1U << (fid - 96));
  }
  return ret;
}
